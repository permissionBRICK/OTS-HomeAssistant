...
def test_scans_selected_private_subnets_and_saved_nat_subnet(net):
    adapters = [
        ...
        {"enabled": True, "ipv4": [{"address": "10.200.0.1", "network_prefix": 8}]},
    ]
    assert net.scan_hosts(adapters) == ["192.168.1.2"]
    hosts = net.scan_hosts(adapters, "192.168.50.80")
    assert hosts[0] == "192.168.50.1"
    assert "192.168.50.80" in hosts
