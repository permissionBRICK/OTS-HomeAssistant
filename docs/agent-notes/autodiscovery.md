...
- `pol*` and `00A003*` are not Ochsner-specific or universal. Siemens's
  ...
  shows POL648_ABCDEF with 00-A0-03-AB-CD-EF. The live AIRHAWK controller uses
...
- Live read-only scan of the project owner's 198.51.100.0/24 found one AIRHAWK518C11A at
  198.51.100.80 in 60.2 seconds. A deliberately unreachable starting connection
...
- Actual router DHCP reassignment and discovery in the project owner's running HA UI
